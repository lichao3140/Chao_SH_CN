...
public class MyApplication extends Application {
    ...
    private void copyActiveFile() {
        ...
        if (ff != null) {
            // 激活文件备份出来
            FileUtils.copyFileUsingFileChannels(ff,Environment.getExternalStorageDirectory() + "/FaceAndroid/.asf_install.dat");
        } else if (ff == null && FileUtils.isFileExist(Environment.getExternalStorageDirectory() + "/FaceAndroid/.asf_install.dat")){
            // 无网络情况下复制到File文件夹里
            FileUtils.copyFileUsingFileChannels(Environment.getExternalStorageDirectory() + "/FaceAndroid/.asf_install.dat", getFilesDir().getAbsolutePath() + File.separator + ".asf_install.dat");
        } else {
            Toast.makeText(this, "算法初始化失败,没有激活文件", Toast.LENGTH_LONG).show();
        }
    }
